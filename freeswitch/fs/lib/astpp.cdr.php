...
function process_cdr($data, $db, $logger, $decimal_points)
{
    ...
    if ($actual_duration > 0) {
            ...
            if(isset($package_array['package_id']))
            {
                ...
                $dataVariable['freeseconds'] = $freeseconds;
            }
            $dataVariable['corrected_duration'] = $package_array['corrected_duration'];
    }
    ...
    if ($debit > 0 && ($dataVariable['calltype'] != "FREE" || $dataVariable['calltype'] != "PACKAGE")) {
...
{
    $corrected_duration = get_corrected_duration($duration, $rate_info);

    $package_array = array();
    $custom_destination = number_loop($destination_number,"patterns",$db);

    $query = "SELECT * FROM packages AS P INNER JOIN package_patterns AS PKGPTR on P.id = PKGPTR.package_id INNER JOIN packages_to_account AS PKGTOAC ON P.id = PKGTOAC.packages_id WHERE ".$custom_destination." AND status = 0 AND PKGTOAC.accountId = ".$accountid." ORDER BY LENGTH(PKGPTR.patterns) DESC LIMIT 1";
    ...
    if($package_info) {
        ...
        if( ($package_info['applicable_for'] == "0" && $call_direction == "outbound") || ($package_info['applicable_for'] == "1" && $call_direction == "inbound") || ($package_info['applicable_for'] == "2") ) {
            ...
            if(!$counter_info) {
                ...
                $db->run($Insert_Query);
                // Query just to get counter_id. Yes, it uses.
                $counter_info =  get_counters($accountid,$package_info['package_id'],$db,$logger);
    ...
    $package_array['corrected_duration'] = $corrected_duration;
...
function get_corrected_duration($duration, $rate_info) {
