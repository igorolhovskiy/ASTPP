...
function normalize_callerid_ani(callerid)
    ...
    if (callerid ~= nil) then
        ...
        local return_cid_name = result['cid_name'] or "<empty>"
        local return_cid_number = result['cid_number'] or "<empty>"

        Logger.debug("[NORMALIZE_CALLERID_ANI] CallerID name: ".. return_cid_name .. ", number: " .. return_cid_number)
...
function neotel_number_normalization_old(xml, destination_number, calleridinfo)
...
function custom_inbound_5(xml, didinfo, userinfo, config, xml_did_rates, callerid_array, livecall_data)
    ...
    Logger.notice("[CUSTOM_INBOUND_5_OVERRIDE] Destination number is " .. destination_number)
...
function get_daily_limit(userinfo)
    return nil
end
...
function get_user_daily_balance(userinfo)
    ...
    if not (userinfo['id'] and tonumber(userinfo['id']) > 0) then
...
function get_balance(userinfo, rates, config)
    ...
    if (userinfo['type'] == '3' and call_direction == 'inbound') then
        Logger.notice("[GET_BALANCE_OVEERIDE]: Inbound DID. Returning 10000")
        return 10000
    end
    ...
    if (daily_balance <= 0) then
        Logger.warning("[GET_BALANCE_OVEERIDE] Check daily balance for this user!!!!")
    end

    return math.min(balance, daily_balance, single_call_balance)
    --return math.min(balance, single_call_balance)
end
...
function neotel_number_normalization(xml, destination_number, calleridinfo)
    ...
    if (calleridinfo ~= nil) then
        ...
        Logger.warning("[NEOTEL_NUMBER_NORMALIZATION] CallerID_name is " .. callerid_name)
