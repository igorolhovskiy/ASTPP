...
-- Check if call was redirected
local call_context = params:getHeader("Caller-Context")
if (call_context == 'redirected') then
    local effective_destination_number = params:getHeader('variable_effective_destination_number')
    accountcode = get_redirected_call_accountcode(effective_destination_number)
    Logger.info("[Dialplan] This is a redirected call. Need to get which account uses DID "..effective_destination_number)
end
