	...
	elseif (tonumber(didinfo['call_type']) == 1 and didinfo['extensions'] ~= '') then
	 ...
	 elseif (tonumber(didinfo['call_type']) == 3 and didinfo['extensions'] ~= '') then -- SIP-DID part
        ...
        if (config['opensips'] == '1') then
            ...
            table.insert(xml, [[<condition field="${user_data ]]..didinfo['extensions']..[[@${domain_name} var forward_to}" expression="^\+?\d*$" break="on-true">]])
            ...
            table.insert(xml, [[</condition>]])
            -- Actual call to user
            table.insert(xml, [[<condition field="" expression="" break="on-true">]])
