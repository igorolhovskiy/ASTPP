	...
	elseif (tonumber(didinfo['call_type']) == 1 and didinfo['extensions'] ~= '') then
	 ...
	 elseif (tonumber(didinfo['call_type']) == 3 and didinfo['extensions'] ~= '') then -- SIP-DID part
        ...
        if (config['opensips'] == '1') then
            ...
            -- Actual call to user
            table.insert(xml, [[<anti-action application="bridge" data="{sip_contact_user=]]..destination_number..[[}sofia/default/]]..destination_number..[[${regex(${sofia_contact(]]..didinfo['extensions']..[[@${domain_name})}|^[^@]+(.*)|%1)}]]..[["/>]])            
            table.insert(xml, [[</condition>]])			
            table.insert(xml, [[</condition>]])	
