...
class Accounts extends MX_Controller {
	...
	function customer_ipmap_action($action, $accountid, $accounttype, $ipmapid = "") {
		...
		if ($action == "add") {
			if ($add_array ['ip'] != "") {
				...
				if ($slash_pos !== false) {
					...
					$mask = substr($ip, $slash_pos + 1 );
					$ip = substr($ip, 0, $slash_pos);
				} else {
					$add_array ['ip'] = $add_array ['ip'] . '/' .$mask;
				}
				$where = array (
				...
				if ($getdata > 0) {
					...
					$this->opensips->opensips_model->add_address(array(
						'ip_addr' => $ip,
						'mask' => $mask,
						'tag' => $accountid
					));
		...
		if ($action == "delete") {
			...
			$this->opensips->opensips_model->del_all_address_for_account($accountid);
			// adding remaining ips to kamalio
			$query_ip_maps = "left(ip, instr(ip, '/')-1) as ip_addr, ifnull(substring(ip, instr(ip,'/')+1),'32') as mask, '$accountid' as tag";
			$ip_maps = $this->db_model->getSelect ( $query_ip_maps, "ip_map", array('accountid' => $accountid) );
			if (isset ( $ip_maps ) && $ip_maps->num_rows () > 0) {
				$ip_data = $ip_maps->result_array ();
				foreach ($ip_data as $address) {
					$this->opensips->opensips_model->add_address($address);
				}
			}
